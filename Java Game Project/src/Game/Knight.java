...
public class Knight extends Main_Character implements Characters
{
    ...
    // Knight's weapons.
    Weapon weapon_1 = new Weapon(2,100,"Sword");
    Weapon weapon_2 = new Weapon(4,600,"Axe");
    Weapon armor_1 = new Weapon(30,100,"Helmet");
    Weapon armor_2 = new Weapon(60,600,"Chestplate");
    Weapon golden_apple = new Weapon(0.5,50,"Golden Apple");

    public Knight(double knight_health, double knight_damage,int secilen_karakter)
    {
        super(knight_health,knight_damage,secilen_karakter);
    }
    ...
    public void Buy_Item(int girilen)
    {
        checker = true;
        this.shop(coin);
        ...
        if (coin >= 50 && girilen == 6) {
            if(backpack_checker)
            {   System.out.println("-------" + weapones.length);
